...
func (v *IconView) GetPathAtPos(x, y int) *TreePath {
	...
	if cpath != nil {
		path = &TreePath{cpath}
		runtime.SetFinalizer(path, func(v *TreePath) { FinalizerStrategy(v.free) })
	}
...
func (v *IconView) GetVisibleRange() (*TreePath, *TreePath) {
	...
	if cpathStart != nil {
		...
		runtime.SetFinalizer(pathStart, func(v *TreePath) { FinalizerStrategy(v.free) })
	}

	if cpathEnd != nil {
		pathEnd = &TreePath{cpathEnd}
		runtime.SetFinalizer(pathEnd, func(v *TreePath) { FinalizerStrategy(v.free) })
	}
...
func (v *IconView) GetTooltipContext(x, y int, keyboardTip bool) (*TreeModel, *TreePath, *TreeIter) {
	...
	if citer != nil {
		iter = &TreeIter{*citer}
		runtime.SetFinalizer(iter, func(v *TreeIter) { FinalizerStrategy(v.free) })
	}
